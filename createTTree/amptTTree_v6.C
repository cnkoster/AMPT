...
void amptTTree_v6(Int_t Njobs = 20, Int_t Nsplit = 4, Int_t nEventsperJob = 100){
    // Global variables
    ...
    for (string cent : Centrality){
        for (Int_t batch = 0; batch<Nsplit; batch++) {
            ...
            string treePath = Form("TreeOutput_Group%i-%i_SpecFalse.root", beginJob-1,endJob-1);
            ...
            for (Int_t i=beginJob; i<=endJob; i++){
                ...
                string Path = Form("/dcache/alice/nkoster/PhD/AMPT_out/Run2_Energy_PbPb/nEvents100/Group");
                ...
                while (fgets(line,81,fp)){
                    ...
                    if (j==eventEnd){
                        if (j!=0){
                            std::cout << "particle stores " << int(particle.size()) << " particles.\n";
                            cout<<"nTrack - specCount is: "<<nTrack-specCount<<"\n";
                            
                            event = Event(nTrack-specCount,b,nPartP,nPartT,nElP,nInelP,nElT,nInelT,particle); //LET OP ALLES ZONDER SPECTATORS
                            eventTree->Fill();
                            particle.clear();
                            //cout<<"EventTree is filled!"<<"\n";
                        }
                        specCount=0;
                    ...
                    hPtDist->Fill(part.getPt());
                    }
                    if(specFlag){specCount++;}
                    j++;
