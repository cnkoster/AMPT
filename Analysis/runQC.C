...
void runQC(Double_t gCentrality=1.0, Int_t iGroupMin=0, Int_t iGroupMax=1000, Double_t gSpectatorMin=0., Double_t gSpectatorMax=500) {
   ...
   // tree->AddFile(Form("5.02TeV/Centrality%s/tree_PaperPreProduction_Group%d.root",centrality.Data(),i));
    ...
    //std::cout<<"We have "<<nevent<<" events"<<std::endl;
    ...
    CalculateQC *fQC = new CalculateQC("CalculateFlowQC");

    
    fQC->UserCreateOutputObjects();
    fQC->SetmaxPtCut(15.0); //pt max for v2 v3 v4
    fQC->SetminPtCut(0.2); // pt min for v1 v2 v3 v4
    ...
    TFile *fResultsFile = new TFile(Form("AnalysisResults_QC_GF_EtaDiff_Group%i-%i_v1-4_ImpactParam.root",iGroupMin, iGroupMax),"RECREATE");
